...
/**
 * Creates a hash of str with cookie secret.
...
function hash (str) {
	...
	// hash using sha256
	return crypto
		.createHmac('sha256', app.get('driveondash'))
		.update(str)
		.digest('base64')
		.replace(/\=+$/, '');
}
...
function signinWithUser (user, req, res, onSuccess) {
	if (arguments.length < 4) {
		throw new Error('session.signinWithUser requires user, req and res objects, and an onSuccess callback.');
	}
	if (typeof user !== 'object') {
		throw new Error('session.signinWithUser requires user to be an object.');
	}
	if (typeof req !== 'object') {
		throw new Error('session.signinWithUser requires req to be an object.');
	}
	if (typeof res !== 'object') {
		throw new Error('session.signinWithUser requires res to be an object.');
	}
	if (typeof onSuccess !== 'function') {
		throw new Error('session.signinWithUser requires onSuccess to be a function.');
	}
	req.session.regenerate(function () {
		req.user = user;
		req.session.userId = user.email;
		// if the user has a password set, store a persistence cookie to resume sessions
		if (app.get('cookie signin') && user.password) {
			var userToken = user.email + ':' + hash(user.password);
			var cookieOpts = _.defaults({}, app.get('cookie signin options'), {
				signed: true,
				httpOnly: true,
				maxAge: 10 * 24 * 60 * 60,
			});
			res.cookie('driveon.uid', userToken, cookieOpts);
		}
		onSuccess(user);
	});
}
...
var postHookedSigninWithUser = function (user, req, res, onSuccess, onFail) {
	app.callHook(user, 'post:signin', function (err) {
		if (err) {
			return onFail(err);
		}
...
exports.persist = function (req, res, next) {
	...
	if (app.get('cookie signin') && !req.session.userId && req.signedCookies['driveon.uid'] && req.signedCookies['driveon.uid'].indexOf(':') > 0) {
		exports.signin(req.signedCookies['driveon.uid'], req, res, function () {
			next();
		}, function (err) {
			var cookieOpts = _.defaults({}, app.get('cookie signin options'), {
				signed: true,
			...
			// Force the cookie to expire!
			cookieOpts.maxAge = 0;
			res.clearCookie('driveon.uid', cookieOpts);
			req.user = null;
			next();
		});
	} else if (req.session.userId) {
		User.find({email:req.session.email}).exec(function (err, user) {
